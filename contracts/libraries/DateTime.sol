...
library DateTime {
    ...
    /// @dev 获取指定时间戳的年月日
    function getDate(uint timestamp) internal pure returns (uint) {
        return uint256(DateTime.getYear(timestamp)) * 10000 + uint256(DateTime.getMonth(timestamp))*100 + uint256(DateTime.getDay(timestamp));
    }
